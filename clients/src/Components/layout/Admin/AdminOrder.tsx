...
const AdminOrder = () => {
  ...
  const handleComplete = async (orderId: number) => {
    const response = await publicAxios.put(`/api/v1/order/${orderId}`);
    if (response.data.status === 200) {
  ...
  const handleCancel = async (orderId: number) => {
    const response = await publicAxios.patch(`/api/v1/order/${orderId}`);
    if (response.data.status === 200) {
      notification.success({
        message: "Cập nhật trạng thái thành công",
      });
  ...
  const handleDeleteProduct = async (orderId: number) => {
    const response = await publicAxios.delete(`/api/v1/order/${orderId}`);
    if (response.data.status === 200) {
      notification.success({
